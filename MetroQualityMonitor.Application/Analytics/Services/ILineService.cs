...
public interface ILineService
{
    ...
    /// </summary>
    Task<IReadOnlyCollection<LineDto>> GetAllAsync(CancellationToken ct = default);

    /// <summary>
    /// Возвращает детальную информацию о линии по идентификатору.
    /// </summary>
    Task<LineDetailsDto?> GetByIdAsync(short id, CancellationToken ct = default);

    /// <summary>
    /// Возвращает список станций линии.
    /// </summary>
    Task<IReadOnlyCollection<StationLiteDto>> GetStationsAsync(short lineId, CancellationToken ct = default);

    /// <summary>
    /// Возвращает квартальный пассажиропоток по линии с возможностью фильтрации по диапазону лет.
    /// </summary>
    Task<IReadOnlyCollection<FlowRecordDto>> GetFlowAsync(short lineId, int? fromYear, int? toYear, CancellationToken ct = default);
}
